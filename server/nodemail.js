...
let activateOptions = (email, id) => ({
	from: `"hava-crm" <${process.env.EMAIL_USER}>`, // sender address
	...
	<button style="color:#fff;border:none;border-radius:10px;padding:1rem 3rem;background-color:#8595ad;font-size:1rem">
	Activate
			</button>
			</a>`
});
...
let acceptManager = (email, password, name, id, manager) => ({
	...
	<button style="border:none;border-radius:10px;padding:1rem 3rem;background-color:#8595ad;font-size:1rem">
				<a href="https://hava-crm.herokuapp.com/api/useraccept?id=${id}&mid=&${manager}" 
